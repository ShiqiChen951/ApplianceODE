...
def main(args):  
    ...
    print("BC_time:",data['BC_time'].shape)
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            print("x is dict with keys:", list(x.keys()))
            for k, v in x.items():
                if torch.is_tensor(v):
                    print(f"x['{k}'] shape: {tuple(v.shape)}, dtype: {v.dtype}, device: {v.device}")
                else:
                    print(f"x['{k}'] type: {type(v)}")
            print("=====================================================")
            printed = True
            # exit()
